...
from business_analyzer import BusinessAnalyzer # Added import
...
analyzer = BusinessAnalyzer() # Initialize business analyzer
...
menu = st.sidebar.selectbox(
    "Navigation",
    ["Single Email Verification", "Bulk Verification", "Email Lists", "API Keys", "Help", "Cookie Settings", "Business Analytics"] #added Business Analytics
)
...
if menu == "Single Email Verification":
...
elif menu == "Business Analytics": # Added Business Analytics section
    st.title("Business Analytics")
    st.markdown("### 📊 Business Analytics")

    # Get verification history (replace with your actual data source)
    all_history = verification_manager.get_all_verification_history()
    if not all_history:
    ...
    if date_filter:
        filtered_df = history_df[history_df['verification_date'].dt.date == date_filter]
    else:
        filtered_df = history_df


    # Calculate business metrics
    business_metrics = analyzer.calculate_business_metrics(filtered_df)
    engagement = analyzer.analyze_engagement(filtered_df)
    ...
    with metrics_container:
        ...
        with col1:
            st.metric("Daily Volume", f"{business_metrics['communication_volume']['daily']:.1f}")
            st.metric("Active Contacts", engagement['active_contacts'])

        with col2:
            st.metric("Response Rate", f"{engagement['engagement_rate']:.1f}%")
            st.metric("Avg Response Time", f"{engagement['avg_response_time']:.1f}h")

        with col3:
            st.metric("Peak Day", business_metrics['peak_activity']['day'])
            st.metric("Peak Hour", f"{business_metrics['peak_activity']['hour']:02d}:00")
